...
We model the KEK as a function of the user's authenticators.
$\mathbf{P}$ = passphrase bytes; $\mathbf{H}$ = FIDO2 hmac-secret
output (32 B); $\mathbf{K}$ = ML-KEM shared secret (32 B).

**Passphrase only.**

$$
\mathrm{KEK} \;=\; \mathrm{Argon2id}\!\left(\mathbf{P},\, \mathrm{salt},\, m_{\mathrm{cost}}, t_{\mathrm{cost}}, p_{\mathrm{cost}}\right)
$$

**FIDO2 wrap mode** (passphrase optional second factor).

$$
\mathrm{KEK} \;=\; \mathrm{Argon2id}\!\left(\texttt{"lbx:fido"} \,\Vert\, \mathbf{P} \,\Vert\, \mathbf{H},\, \mathrm{salt}\right)
$$

**FIDO2 direct mode** (no wrap; MVK derived purely from the device).

$$
\mathrm{MVK} \;=\; \mathrm{HKDF\text{-}SHA256}\!\left(\mathbf{H},\, \mathrm{salt},\, \texttt{"lbx:mvk-fido/v1"}\right)
$$

Note: this mode has **no wrapped MVK on disk**. Lose the device, lose
the vault. Trade-off: nothing for an offline brute-force attacker
to grind on.

**Hybrid passphrase + ML-KEM-768/1024.**

$$
\mathrm{KEK} \;=\; \mathrm{HKDF\text{-}SHA256}\!\left(\,\mathrm{Argon2id}(\mathbf{P}) \,\Vert\, \mathbf{K},\; \mathrm{salt},\; \texttt{"lbx:hybrid-kek/v1"}\right)
$$

**Hybrid FIDO2 + ML-KEM-768/1024.** This is the hybrid that closes
the actual PQ gap, because ECDH-P256 inside CTAP2 is the *only*
asymmetric primitive on the FIDO2 wire, quantum-recoverable from
sniffed USB-HID traffic by a CRQC adversary. Mixing in $\mathbf{K}$
means the recorded transport doesn't help the adversary alone:

$$
\mathrm{KEK} \;=\; \mathrm{HKDF\text{-}SHA256}\!\left(\mathbf{H} \,\Vert\, \mathbf{K},\; \mathrm{salt},\; \texttt{"lbx:hybrid-fido-kek/v1"}\right)
$$

The ML-KEM shared secret $\mathbf{K}$ comes from decapsulating the
ciphertext stored in the `.hybrid` sidecar against the user's `.kyber`
seed file, kept on **separate trusted storage**.
...
$$
\mathrm{ct}_i \;=\; \mathrm{AEAD}\!\left(\mathrm{file\_key},\, \mathrm{nonce}_i,\; \mathrm{aad} = (\mathrm{file\_id} \,\Vert\, i \,\Vert\, g),\; \mathrm{plaintext}_i\right)
$$
...
`file_id`), and an old version of a chunk cannot be replayed
(different $g$). Tested in
`crates/luksbox-vfs/tests/security_invariants.rs`.

### 3.4 Slot AAD coverage + layout (V1 / V2 / V3)

Pre-V2 (audit round 6 noted this), the slot's AEAD AAD covered offsets
`0..76` of the slot bytes, kind, uuid, kdf_params, kdf_salt,
aead_nonce. The cred_id and hmac_salt fields lived at `124..288` and
were authenticated only by the header HMAC. V2 widens the slot AEAD
AAD to also cover those fields:

$$
\mathrm{aad}_\text{V2} \;=\; \mathrm{slot}[0..76] \;\Vert\; \mathrm{slot}[124..288] \;\Vert\; \mathrm{header\_salt}
$$

V3 (current default) repurposes the previously-padding region 288..512
to extend the cred_id capacity from 128 to 352 bytes, accommodating
FIDO2 authenticators that produce cred IDs larger than the typical
YubiKey 64-byte case. Reported sizes: Google Titan 288 B, SoloKey
stateless mode 140 B; the exact format varies per vendor and isn't
always publicly documented. V3 layout: cred_id at offsets 128..480,
hmac_salt at 480..512. AAD scope expands to match:

$$
\mathrm{aad}_\text{V3} \;=\; \mathrm{slot}[0..76] \;\Vert\; \mathrm{slot}[124..512] \;\Vert\; \mathrm{header\_salt}
$$
...
offset 1 selects the AAD shape AND the on-disk layout; future format
extensions get a new version byte at the same location. Existing
V1/V2 vaults remain readable; new slots default to V3.
...
flowchart TB
    ...
    L2[Per-chunk AAD<br/>file_id || chunk_idx || generation]
    L3[Per-file file_key<br/>HKDF MVK, salt, lbx:file/v1: || file_id]
    L4[Per-vault MVK<br/>memfd_secret(2) on Linux >= 5.14<br/>mlock'd Zeroize Box otherwise]
    L5[8 KiB header HMAC<br/>over the entire header incl. every slot]
    L6[Per-slot AEAD wrap, AAD V2<br/>slot 0..76 || slot 124..288 || header_salt]
    ...
    D4[defeats:<br/>MVK exposed via hibernate / coredump]
    D5[defeats:<br/>any tamper of any header byte]
    D6[defeats:<br/>cred_id / hmac_salt swap]

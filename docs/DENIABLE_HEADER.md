...
| Fuzz target (envelope open path) | `fuzz/fuzz_targets/deniable_header_parse.rs` (libfuzzer) + `fuzz-afl/src/bin/deniable_header_parse.rs` (AFL++) | shipped (retargeted at v2 envelope discovery + complete-open) |
| Fuzz targets (slot-payload decoder + round-trip) | `fuzz/fuzz_targets/slot_payload_{decode,roundtrip}.rs` (libfuzzer) + `fuzz-afl/src/bin/slot_payload_{decode,roundtrip}.rs` (AFL++) | shipped (bypass Argon2id, hit `SlotPayload::decode`/`encode` directly) |
| Fuzz seed corpus (shared between libfuzzer + AFL++) | `crates/luksbox-format/examples/gen_fuzz_seeds.rs` writes to both `fuzz/corpus/<target>/` and `fuzz-afl/seeds/<target>/` | shipped |
...
### Fuzz targets (libfuzzer + AFL++)

Each target ships in both fuzzing setups under the same name. See
`FUZZING.md` for the operational playbook (local dev runs use
libfuzzer, sustained server campaigns use AFL++; the two engines'
different mutator personalities have a track record of finding
different bugs against the same surface).

| Target | Surface | Invariants | libfuzzer harness | AFL++ harness |
|---|---|---|---|---|
| `deniable_header_parse` | Full v2 open path (`try_open_envelope_v2` + `complete_open_v2`) with attacker-controlled bytes + passphrase + cipher | Never panics; every failure collapses to `Error::OpaqueUnlockFailed` (no oracle leakage). | `fuzz/fuzz_targets/deniable_header_parse.rs` | `fuzz-afl/src/bin/deniable_header_parse.rs` |
| `slot_payload_decode` | `SlotPayload::decode` directly (bypasses Argon2id, no envelope KEK needed) | Never panics; every rejection is `Error::InvalidField`; successful decode round-trips via `encode` (excluding random padding tail). | `fuzz/fuzz_targets/slot_payload_decode.rs` | `fuzz-afl/src/bin/slot_payload_decode.rs` |
| `slot_payload_roundtrip` | `SlotPayload::new` + `encode` + `decode` with attacker-controlled length triples | Constructor rejections are always justified by a documented cap; successful constructions round-trip field-for-field. | `fuzz/fuzz_targets/slot_payload_roundtrip.rs` | `fuzz-afl/src/bin/slot_payload_roundtrip.rs` |

The shared seed corpus for these three targets is generated by
`cargo run --example gen_fuzz_seeds -p luksbox-format` (the
existing seed-gen example, extended at the end). It writes one
valid input per target into both `fuzz/corpus/<target>/` and
`fuzz-afl/seeds/<target>/` so the two engines start from the same
human-curated regression base. The deniable targets are also
registered in `scripts/fuzz_server.sh`'s `TARGETS` array so the
"run all" path picks them up.
